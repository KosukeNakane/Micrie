...
export const DrumsEditor: React.FC = () => {
	...
	useEffect(() => {
		...
		const hasAssigned = next.some(
			(seg) => seg.label === 'kick' || seg.label === 'snare' || seg.label === 'hihat'
		);
		if (hasAssigned) {
	...
	}, [hasPattern, drumPattern, rhythmSegments, setRhythmSegments]);
	...
	const labels = useMemo(
		() => Array.from({ length: STEPS }, (_, i) => rhythmSegments[i]?.label ?? ''),
		[rhythmSegments]
	);
