...
const hasActivePattern = () => Boolean(useEditingPatternStore.getState().pattern);
...
export const useAudioRecorder = () => {
		...
		mediaRecorder.onstop = () => {
			if (chunksRef.current.length > 0) {
				...
				if (blob.size > 1000) {
					...
					// 共有ストアにも録音由来として反映
					try {
						setSharedAudioBlob(blob, 'recorded');
					} catch {}
					...
					if (mode === 'rhythm') {
						...
						apiFetch('pitch', {
							...
							.then((data) => {
								...
								const finalSegments: Segment[] = data.pitch_series.map(
								...
								setMelodySegments(finalSegments);
							})
							.catch((err) => {
	...
	const stopRecording = async (tempo: number) => {
		...
		mediaRecorderRef.current.onstop = async () => {
			const blob = new Blob(chunksRef.current, { type: 'audio/webm' });
			if (blob.size > 1000) {
				const finalBlob = remaining > 0 ? await appendSilenceToBlob(blob, 44100, remaining) : blob;
				setAudioBlob(finalBlob);
				// 共有ストアにも録音由来として反映
				try {
					setSharedAudioBlob(finalBlob, 'recorded');
				} catch {}
			}
			setIsRecording(false);
		};
